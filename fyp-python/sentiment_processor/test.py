...
while(still_items_left):
    ...
    for item in results["Items"]: 
        try:
            test_item_there = item['article-pub-date']
        except:
            ...
            analysed_topics = articleAnalyser.analyseTopicsSentiment()
            likely_topics = analysed_topics[0]
            article_sentiment_matrix = analysed_topics[1]
            most_similar_party = articleAnalyser.analyseManifestoSimilarity()
            headline_sentiment_matrix = articleAnalyser.analyseHeadlineSentiment()
            ...
            article_sentiment_matrix = dict([(TopicsHelper.topicIndexToTopic[topic_num], topic_sentiment) for topic_num,topic_sentiment in article_sentiment_matrix.items() if topic_num != 0])
            article_sentiment_matrix = ", ".join([topic + " = " + str(score) for topic, score in article_sentiment_matrix.items()])

            most_similar_party = PoliticalPartyHelper.enumToPoliticalPartyString[most_similar_party]

            headline_sentiment_matrix = dict([(TopicsHelper.topicIndexToTopic[topic_num], topic_sentiment) for topic_num,topic_sentiment in headline_sentiment_matrix.items() if topic_num != 0])
            headline_sentiment_matrix = ", ".join([topic + " = " + str(score) for topic, score in headline_sentiment_matrix.items()])
            ...
            if (article_sentiment_matrix == ""):
                article_sentiment_matrix = "NO INFO"
            if (most_similar_party == ""):
                most_similar_party = "NO INFO"
            if (headline_sentiment_matrix == ""):
                headline_sentiment_matrix = "NO INFO"
            if (top_words == ""):
                top_words = "NO INFO"
            ...
            response = table.update_item( # update database entry with new text and metadata
                ...
                UpdateExpression="SET #ap=:p, #atp=:t, #ats=:s, #mlp=:m, #hts=:h, #tw=:w",
                ExpressionAttributeValues={
                    ':p': article_pub_date,
                    ':t': likely_topics,
                    ':s': article_sentiment_matrix,
                    ':m': most_similar_party,
                    ':h': headline_sentiment_matrix,
                    ':w': top_words,            
                },
